...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$style(HTML(
      ...
      .nav-tabs > li > a { color: #0056b3; font-weight: 600; }
      .nav-tabs > li.active > a, .nav-tabs > li.active > a:focus { background-color: #0056b3; color: #ffffff; }
...
server <- function(input, output, session) {
  ...
  output$predicciones <- renderDT({
    ...
    falt <- setdiff(vars, colnames(drv_df)); drv_df[falt] <- 0
